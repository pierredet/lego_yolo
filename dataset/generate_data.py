...
"""Generate a new dataset by randomly combining modified version of an item
with a background.
...
def _random_rotation(im):
    ...
    corners = np.array([[0, 0, 1], [cols - 1, 0, 1],
                        [0, rows - 1, 1], [cols - 1, rows - 1, 1]])
...
def _random_perspective(im):
    ...
    corn1 = pts1[0, :] + \
        [np.random.randint(colDist), np.random.randint(rowDist)]
    corn2 = pts1[1, :] + \
        [-np.random.randint(colDist), np.random.randint(rowDist)]
    corn3 = pts1[2, :] + \
        [-np.random.randint(colDist), -np.random.randint(rowDist)]
...
def _random_contrast_brightness(im):
    ...
    alpha = (1 - MAX_CONTRAST_DEV) + 2 * \
        MAX_CONTRAST_DEV * np.random.rand(1)[0]
...
def _item_background_merging(imItem, imBack, showsteps=False):
    ...
    if showsteps:
        cv2.imshow('1 - transparent', imItem)
    imItem = _random_flip(imItem)
    if showsteps:
        cv2.imshow('2 - flip', imItem)
    imItem = _random_rotation(imItem)
    if showsteps:
        cv2.imshow('3 - rotation', imItem)
    imItem = _random_scale(imItem)
    if showsteps:
        cv2.imshow('4 - scaling', imItem)
    imItem = _random_perspective(imItem)
    if showsteps:
        cv2.imshow('5 - perspective', imItem)
    imItem = _random_HSV(imItem)
    if showsteps:
        cv2.imshow('6 - hsv', imItem)
    imItem = _random_contrast_brightness(imItem)
    if showsteps:
        cv2.imshow('7 - contrast and brightness', imItem)
    imItem = _final_crop(imItem)
    if showsteps:
        cv2.imshow('8 - final crop', imItem)
...
def _generate_xml(
        filename, width, height, depth, label, xmin, ymin, xmax, ymax):
